...
function create_dae_file {
   ...
   cp $TMP_DIR/robot.dae $(dirname "$0")/robot.dae
}
...
EOF
   ...
   cp $TMP_DIR/wrapper.xml $(dirname "$0")/wrapper.xml
   cp $(ls -1 $TMP_DIR/.openrave/*/*) $(dirname "$0")/  
   mv $(dirname "$0")/ikfast*.cpp $(dirname "$0")/ikfast61.cpp  
}
...
if [ "$KEEP_RESULTS" == "1" ] ; then
   echo "Storing intermediate results in: $(dirname $0)"
else
